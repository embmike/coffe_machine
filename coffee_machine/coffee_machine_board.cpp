...
namespace
{
   ...
   static PixelRGB888 frame_buffer[kLcdWidth * kLcdHeight]; ///< Bring-up framebuffer used for color bars and LTDC tests.
   ...
   void drawColorBars()
   {
      static const PixelRGB888 colors[3] = {
         ...
         { 0x00U, 0xFFU, 0x00U },
         { 0xFFU, 0x00U, 0x00U }
      }; ///< RGB888 test bars for basic LTDC and SDRAM validation.
   ...
   HAL_StatusTypeDef sdramSelfTest()
   {
      ...
      uint32_t* const cache_buffer = reinterpret_cast<uint32_t*>(kFramebufferAddress); ///< Non-volatile pointer for cache maintenance calls.
      static const uint32_t patterns[] = {
         ...
         0x12345678U,
         0x89ABCDEFU
      }; ///< Test pattern set for a quick SDRAM sanity check.
